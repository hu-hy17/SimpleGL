...
QMatrix4x4 Camera::getProjectionMatrix()
{
	...
	projection.perspective(zoom, 1.0f * screen_width / screen_height, 0.1f, 10000.f);
	return projection;
}

GLRay Camera::getRay(int px, int py)
{
	float resolution = 1.0f / std::max(screen_width, screen_height);
	QVector3D dir = (ZOOM / zoom) * front + right * (2 * (px + 0.5f) * resolution - 1) + up * (2 * (py + 0.5f) * resolution - 1);
	return GLRay(position, dir);
}
